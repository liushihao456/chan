...
def check_bi(bars: List[NewBar], benchmark: float = None):
    ...
    if benchmark and abs(fx_a.fx - fx_b.fx) > benchmark * config.bi_change_th:
        power_enough = True
    else:
        power_enough = False
...
class CZSC:
    ...
    def __update_bi(self):
        ...
        if config.bi_change_th > 0.5 and len(self.bi_list) >= 5:
